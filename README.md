...
Tabulation System for Year-End Party's Class Performance 
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/class-presentation`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `class-presentation` and import [class-presentation.sql](class-presentation.sql) into it.

6. Open the terminal and navigate to the project directory **class-presentation**.
...
9. Open your web browser and access <http://localhost:5190/class-presentation/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-class-presentation` and import [class-presentation.sql](class-presentation.sql) into it.
...
2. Access the application by visiting `http://[host_name]/class-presentation`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/class-presentation>
     - <http://192.168.1.99/class-presentation>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/class-presentation/app>.
